...
const SMARTFLO_API_URL = process.env.SMARTFLO_API_URL || "https://api-smartflo.tatateleservices.com/v1/click_to_call_support";
...
console.log("URL -- ", SMARTFLO_API_URL)
const router = Router();
...
router.post('/make-call', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    // If contactId is provided, fetch phone number from DB
    if (contactId) {
      const db = await DatabaseConnection.getConnection();
    ...
    let formattedNumber = numberToCall.replace(/[^0-9]/g, '');
    if (formattedNumber.length === 10) {
      formattedNumber = '91' + formattedNumber; // default India country code
    }

    // Call Smartflo Click-to-Call Support API
    const response = await axios.post(SMARTFLO_API_URL,
      {
